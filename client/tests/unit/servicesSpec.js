  ...
  describe('Auth', function() {
    describe('instantiate', function() {
      ...
      it('should have signup function', function() {
        ...
        expect(angular.isFunction(Auth.signup)).to.equal(true);
      });
      ...
      it('should set the user object with no name and public role', function() {
        expect(Auth.user).to.deep.equal({ emailAddress: '', role: Auth.userRoles.public });
      });
    });
    ...
    describe('signup', function() {
      ...
      it('should append the user', function() {
        var success = function() {};
        var error = function() {};
        $httpBackend.expectPOST('/signup').respond({ 'user': 'foo' });
        Auth.register({}, success, error);
        $httpBackend.flush();
        expect(Auth.user).to.deep.equal({ emailAddress : '', role : { bitMask : 1, title : 'public' }, user : 'foo' });
      });
    });
    ...
    describe('login', function() {
      ...
      it('should append the user', function() {
        ...
        Auth.login({}, success, error);
        $httpBackend.flush();
        expect(Auth.user).to.deep.equal({ emailAddress : '', role : { bitMask : 1, title : 'public' }, user : 'bar' });
      });
    });

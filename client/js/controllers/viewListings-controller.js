...
['$rootScope', '$timeout', '$scope', '$location', '$window', 'Auth', 'PropertyListing', 'leafletData', '$modal', function($rootScope, $timeout, $scope, $location, $window, Auth, PropertyListing, leafletData, $modal) {
    ...
    $scope.changeMarkerOn = function(property, index)
    {

        drawHoverMarker(property.lat, property.lon, property._id);
        $scope.properties[index].test = !property.test;
    };

    $scope.changeMarkerOff = function(property, index)
    {
    ...
    function renderProperties(_properties)
    {
        ...
        $scope.properties = new Array();
        $rootScope.myProperties = new Array();
        ...
        for (var i = 0; i < _properties.length; i++)
        {
            ...
            if (addedAlready === false)
            {
                ...
                // console.log(prop['addedImages']);

                if(prop['addedImages'].length > 0){
                    prop['src'] = prop['addedImages'][0].url;
                } else {
                    prop['src'] = '/img/listing/editPropertyPlaceholder.png';
                }

                $scope.properties.push({'_id': prop['_id'], 'addedProfileImages': prop['addedProfileImages'], 'name': prop['name'], 'ageGroup':prop['ageGroup'], 'lifestyle':prop['lifestyle'], 'property': prop, 'propertyType': prop['propertyType'], 'price': prop['rentAmount'], 'test': false,'title': prop['title'], 'published': prop['published'], 'publishedAt': prop['publishedAt'], 'src': prop['src'], 'daysAgo': daysAgo, 'bedrooms':prop['bedrooms'], 'bathrooms':prop['bathrooms'], 'lon': prop['lon'], 'lat': prop['lat'], 'hoverButtons': false});
                $rootScope.myProperties = $scope.properties;

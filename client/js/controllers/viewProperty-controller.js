...
['$rootScope', '$timeout', '$scope', '$location', '$window', 'leafletData', 'PropertyListing', '$modal', 'Auth', '$stateParams', 'baseUrl', '$anchorScroll', 'QuestionsListing', function($rootScope, $timeout, $scope, $location, $window, leafletData, PropertyListing, $modal, Auth, $stateParams, baseUrl, $anchorScroll, QuestionsListing) {


    $scope.property = new Object();
    $scope.questionsListing = new Object();
    ...
    function compareQuestions() {
      ...
      function(res) {
        ...
        $scope.roommateQuestions = $scope.property.questionsResults;
        $scope.userQuestions = $scope.user.questionsResults;
    ...
    function initPropertyListing(){
        ...
        if( typeof $scope.property.thumb == 'undefined' ){
            var url = $scope.property.addedProfileImages[0].url.split('upload/');
            $scope.property.thumb = url[0] + 'upload/w_400,h_400,c_fill,g_face/' + url[1];
        }

        // $scope.property.thumb = 'http://res.cloudinary.com/apartmate/image/upload/w_200,h_200,c_fill,g_face/v1447797444/iblvedpc3v8jgqqajpld.jpg';
        // $scope.property.portrait = 'http://res.cloudinary.com/apartmate/image/upload/w_400,h_400,c_fill,g_face/v1447797444/iblvedpc3v8jgqqajpld.jpg';
        
        if ($scope.property.createdAt && $scope.property.createdAt != '' && $scope.property.published)
        {
            var formattedPostDate = $scope.property.createdAt;
            ...
            $scope.property.daysAgo = Math.floor((Date.now() - formattedPostDate) / 86400000);
        }
        ...
        $scope.selectedPropertyImage = $scope.property.addedImages[$scope.selectedPropertyIdx].url;
        ...
        angular.extend($scope, {
            toronto: {
                lat: $scope.property.location.lat,
                lng: $scope.property.location.lon,
                zoom: 15
            },
            tiles: tilesDict.mapbox_terrain,
            markers: {},
    ...
    function setDaysAgo(_publishedAt, _published){
        if (_publishedAt && _publishedAt != '' && _published)
        {
              var formattedPostDate = _publishedAt;
              formattedPostDate = formattedPostDate.replace(/-/g, "/");
              formattedPostDate = formattedPostDate.replace(/T.*$/, "");
              formattedPostDate = Date.parse(formattedPostDate);
              // prop['publishedAt'] = formattedPostDate;
              return Math.floor(( Date.now() - formattedPostDate) / 86400000);
        } 
    }

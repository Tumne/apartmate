...
['$rootScope', '$timeout', '$scope', '$location', '$window', 'Auth', 'PropertyListing', 'leafletData', '$modal', function($rootScope, $timeout, $scope, $location, $window, Auth, PropertyListing, leafletData, $modal) {

    $rootScope.seo = {
      title: "Apartmate | My Favorites",
    ...
    function drawSavedNeighbourhoods()
    {
        var layer;
        var neighborhoodSelected = false;

        //console.log('n: ' + $scope.selectedNeighbourhoods.length);

        for (var i = 0; i < $scope.selectedNeighbourhoods.length; i++)
        {
            layer = $scope.selectedNeighbourhoods[i];
                layer.setStyle({
                weight: 4,
                opacity: 0.8,
    ...
    $scope.removeFavorite = function(property) {
    ...
    $scope.onViewPropertySelect = function(_property)
    {
        ...
        console.log("TEST");
        $location.path('/favorite_property/'+_property._id).replace();
    }
    ...
    function initializeLeaflet()
    {
        ...
        leafletData.getMap().then(function(map) 
        {
            ...
            // for (var i = 0; i < $scope.selectedNeighbourhoods.length; i++)
            // {
            //     var layer = $scope.selectedNeighbourhoods[i];
            //     layer.setStyle({
            //         weight: 4,
            //         opacity: 0.8,
            //         fillOpacity: 0.60000000298023224
    ...
    function drawMarker(lat, lon, _id) {
        leafletData.getMap().then(function(map) 
        {
    ...
    $scope.changeMarkerOn = function(property)
    {
        ...
        drawHoverMarker(property.lat, property.lon, property._id);
    };

    $scope.changeMarkerOff = function(property)
    {
        //console.log('property off:' + property.lon);
        hideHoverMarker(property.lat, property.lon, property._id);
    };
    ...
    function renderProperties(_properties)
    {
        ...
        for (var i = 0; i < _properties.length; i++)
        {
            ...
            if (prop['images'] && prop['images'] != '' && prop['images'] != '[]')
            {
                ...
                prop['favoritedByThisUser'] = false;
                if ($rootScope.user.favoriteProperties && $rootScope.user.favoriteProperties.length > 0)
                {
                   for (var f = 0; f < $rootScope.user.favoriteProperties.length; f++)
                   {
                    console.log($rootScope.user.favoriteProperties[f].id);
                    console.log(prop['_id']);

                    if ($rootScope.user.favoriteProperties[f].id == prop['_id'])
                    {
                      
                      prop['favoritedByThisUser'] = true;
                    }
                   }
                }               
                ...
                if (prop['email'] !== '' && prop['title'] != '' && prop['description'] && prop['bedrooms'] != '' && locationGiven == true && basementPassed === true && petsPassed === true)
                {
                    ...
                    for (var s = 0; s < $scope.properties.length; s++)
                    {
                        ...
                        {
                            $scope.properties[s].favoritedByThisUser = prop['favoritedByThisUser'];
                            addedAlready = true;
                        }
                    }
                    ...
                    if (addedAlready === false)
                    {
                        ...
                        $scope.properties.push({'_id': prop['_id'], 'favoritedByThisUser': prop['favoritedByThisUser'], 'UserpostId': prop['postId'], 'src': myImageObject[0], 'phone': prop['phone'], 'email': prop['email'], 'daysAgo': daysAgo, 'formattedAvailableDate': formattedAvailableDate, 'postDate': prop['postDate'], 'bathroom': prop['bathrooms'], 'bedroom': prop['bedrooms'], 'tags': prop['tags'], 'pets': prop['pets'], 'couples': prop['couples'], 'basementApartment': prop['basementApartment'], 'price': prop['rentAmount'], 'description': prop['description'], 'title': prop['title'], 'lon': prop['lon'], 'lat': prop['lat'], 'allImages': allImages });
            ...
            } else if (prop['addedImages'] && prop['addedImages'] != '' && prop['addedImages'].length > 0)
            {
                ...
                var allImages = new Array();
                for (var mo = 0; mo < prop['addedImages'].length; mo++)
                {
                    var exists = false;
                    ...
                    for (var m = 0; m < allImages.length; m++)
                    {
                        ...
                        if (allImages[m].url === prop['addedImages'][mo].url)
                            exists = true;
                    }

                    if (exists === false)
                        allImages.push({ 'url': prop['addedImages'][mo].url });
                }
                if (prop['petsWelcome'] && prop['petsWelcome'].length > 0 )
                {
                    if ((petsCondition === prop['petsWelcome'][0].name) || prop['petsWelcome'][0].name === 'Yes' )
                ...
                if (prop['title'] != '' && prop['bedrooms'] != '' && locationGiven == true && petsPassed === true && basementPassed === true)
                {
                    ...
                    if (addedAlready === false)
                    {
                        $scope.properties.push({'_id': prop['_id'], 'favoritedByThisUser': prop['favoritedByThisUser'], 'allImages': allImages, 'src': prop['addedImages'][0].url, 'phone': prop['phone'], 'email': prop['email'], 'daysAgo': daysAgo, 'formattedAvailableDate': formattedAvailableDate, 'postDate': prop['createdAt'], 'bathroom': prop['bathrooms'], 'bedroom': prop['bedrooms'], 'price': prop['rentAmount'], 'description': prop['description'], 'title': prop['title'], 'lon': prop['lon'], 'lat': prop['lat']});
                       // console.log('$scope.properties length: ' + $scope.properties.length);
                    } else {
                        //console.log('property already added');
                    }
        ...
        //$rootScope.success = "We've got " + $scope.properties.length + " listings.";
        //console.log('Filtered: ' + $scope.properties.length);
        for (var i = 0; i < $scope.properties.length; i++)
        {
        ...
        //     drawSavedNeighbourhoods();
        // else {
        //     console.log('no neighbourhood selected');
        // }
    }

...
describe('Server Integration Tests - ', function (done) {
    ...
    it('Sign up a new user(no role) - Return a 400', function(done) {
        request(app).post('/signup').send(user2).expect(400, done);
    });
    it('Sign up a new user - Return a 200', function(done) {
        request(app).post('/signup').send(user).expect(200, done);
    });
